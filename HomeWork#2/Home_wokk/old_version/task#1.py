...
# # Поэтому я начал делать все через sqlit3, ну и все слетело, осталось только первоначальное решениетавление. Функцию hex используйте для проверки своего результата.
